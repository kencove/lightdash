...
const SPACE_NAME = '[Test SQL Generation]';
...
async function createTestSqlGenerationSpace(
    knex: Knex,
...
export async function seed(knex: Knex): Promise<void> {
    ...
    // Create the [Test SQL Generation] space
    const spaceUuid = await createTestSqlGenerationSpace(knex, spaceModel);

    // Create all POP test charts in that space
    ...
    await createPopTestsDashboard(knex, spaceUuid, chartUuids);
}
