...
const FILTER_TESTS_SPACE_NAME = 'Filter Tests';

type ChartUuids = Record<string, string>;

async function createFilterTestsSpace(
    knex: Knex,
...
export async function seed(knex: Knex): Promise<void> {
    ...
    // Create the Filter Tests space
    const spaceUuid = await createFilterTestsSpace(knex, spaceModel);

    // Create all filter test charts in that space
    const chartUuids = await createFilterTestCharts(savedChartModel, spaceUuid);

    // Create dashboard with all charts
    await createFilterTestsDashboard(knex, spaceUuid, chartUuids);
}
